...
def test_urls_must_be_on_of_amazon_domain():
    ...
    assert str(e.value) == ('Url https://www.amazonial.co.uk/ is invalid. Valid domain are one of \n'
                            'www.amazon.com.au,\n'
                            'www.amazon.com.be,\n'
                            'www.amazon.com.br,\n'
                            'www.amazon.ca,\n'
...
def test_urls_scheme_must_be_https():
    with pytest.raises(InvalidUrlException) as e:
        crawl_job = CrawlJobInput(urls={AnyUrl('http://www.amazon.co.uk')})
        getattr(crawl_job, 'validate_urls')()
    assert str(e.value) == 'Url http://www.amazon.co.uk/ is invalid. Only url with https scheme are valid.'


def test_validate_urls_from_amazon():
    with pytest.raises(InvalidUrlException) as e:
        crawl_job = CrawlJobInput(urls={AnyUrl(
